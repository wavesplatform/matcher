...
import com.wavesplatform.dex.api.ws.entities.{WsLastTrade, WsOrderBookRestrictions, WsOrderBookSettings}
...
object WsOrderBookChanges {
  ...
  final case class LTrade(price: Long, amount: Long, side: OrderType)
  final case class LevelAggregation(price: Long, amount: Long, side: OrderType)
  ...
  def from(
    ...
    amountDecimals: Int,
    priceDecimals: Int,
    asks: Iterable[LevelAggregation],
    bids: Iterable[LevelAggregation],
    lt: Option[LTrade],
    updateId: Long,
    restrictions: Option[WsOrderBookRestrictions],
    tickSize: Double
  ): WsOrderBookChanges =
    WsOrderBookChanges(
      assetPair = assetPair,
  ...
  def lastTrade(amountDecimals: Int, priceDecimals: Int, x: LTrade): WsLastTrade = WsLastTrade(
  ...
  def side(amountDecimals: Int, priceDecimals: Int, xs: Iterable[LevelAggregation], ordering: Ordering[Double]): WsSide =
    TreeMap(
      xs.map { x =>
