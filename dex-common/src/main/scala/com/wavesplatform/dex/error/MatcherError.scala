...
final case class DeviantOrderPrice(orderType: OrderType, orderPrice: Price, deviationSettings: Deviations)
    extends MatcherError(
...
object DeviantOrderPrice extends MatcherErrorCodeProvider(order, price, outOfBound, C.BadRequest) {
  ...
  def apply(ord: Order, deviationSettings: Deviations)(implicit efc: ErrorFormatterContext): DeviantOrderPrice =
    DeviantOrderPrice(ord.orderType, Price(ord.assetPair, ord.price), deviationSettings)

}

final case class DeviantOrderMatcherFee(orderType: OrderType, matcherFee: Amount, deviationSettings: Deviations)
    extends MatcherError(
...
object DeviantOrderMatcherFee extends MatcherErrorCodeProvider(order, fee, outOfBound, C.BadRequest) {
  ...
  def apply(ord: Order, deviationSettings: Deviations)(implicit efc: ErrorFormatterContext): DeviantOrderMatcherFee =
...
final case class OrderInvalidAmount(orderAmount: Amount, amtSettings: OrderRestriction)
    extends MatcherError(
      OrderInvalidAmount.code,
      e"""The order's amount
       |${"amount" -> orderAmount}
       |does not meet matcher's requirements:
       |max amount = ${"max" -> amtSettings.max},
       |min amount = ${"min" -> amtSettings.min},
       |step amount = ${"step" -> amtSettings.step}""",
      OrderInvalidAmount.httpCode
    )

object OrderInvalidAmount extends MatcherErrorCodeProvider(order, amount, denied, C.BadRequest) {

  def apply(ord: Order, amtSettings: OrderRestriction)(implicit efc: ErrorFormatterContext): OrderInvalidAmount =
...
final case class OrderInvalidPrice(orderPrice: Price, prcSettings: OrderRestriction)
    extends MatcherError(
      OrderInvalidPrice.code,
      e"""The order's price
       |${"price" -> orderPrice}
       |does not meet matcher's requirements:
       |max price = ${"max" -> prcSettings.max},
       |min price = ${"min" -> prcSettings.min},
       |step price = ${"step" -> prcSettings.step}""",
      OrderInvalidPrice.httpCode
    )

object OrderInvalidPrice extends MatcherErrorCodeProvider(order, price, denied, C.BadRequest) {

  def apply(ord: Order, prcSettings: OrderRestriction)(implicit efc: ErrorFormatterContext): OrderInvalidPrice =
...
final case class OrderRestriction(min: Double, max: Double, step: Double)
final case class Deviations(maxPriceProfit: Double, maxPriceLoss: Double, maxFeeDeviation: Double)
