...
object EnrichedResponse {
  ...
  sealed trait As[ErrorT, EntityT] {
    def tryGet(response: Response[Either[String, String]]): Either[ErrorT, EntityT]
  }

  class AsJson[ErrorT: Reads, EntityT: Reads] extends As[ErrorT, EntityT] {

    override def tryGet(response: Response[Either[String, String]]): Either[ErrorT, EntityT] = parseWithError(response) { x =>
      Json.parse(x)
        .asOpt[EntityT]
  ...
  class AsHocon[ErrorT: Reads] extends As[ErrorT, Config] {
    ...
    override def tryGet(response: Response[Either[String, String]]): Either[ErrorT, Config] = parseWithError(response) { x =>
  ...
  class Ignore[ErrorT: Reads] extends As[ErrorT, Unit] {
    override def tryGet(response: Response[Either[String, String]]): Either[ErrorT, Unit] = parseWithError(response)(_ => ().asRight)
  }

  def parseWithError[ErrorT: Reads, EntityT](response: Response[Either[String, String]])(f: String => Either[ErrorT, EntityT])
    : Either[ErrorT, EntityT] =
    response.body match {
      case Right(x) => f(x)
      case Left(e) =>
        Json.parse(e)
