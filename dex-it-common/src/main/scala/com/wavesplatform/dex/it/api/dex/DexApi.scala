...
trait DexApi[F[_]] extends HasWaitReady[F] {
  ...
  def tryOrderHistoryWithApiKey(owner: Address, activeOnly: Option[Boolean] = None): F[Either[MatcherError, List[ApiOrderBookHistoryItem]]]
  ...
  def waitForOrderHistory[A](owner: KeyPair, activeOnly: Option[Boolean])(pred: List[ApiOrderBookHistoryItem] => Boolean): F[List[ApiOrderBookHistoryItem]]
  ...
  def apply[F[_]](apiKey: String,
    ...
    new DexApi[F] {
      ...
      override def tryOrderHistory(owner: KeyPair,
                                   activeOnly: Option[Boolean] = None,
                                   timestamp: Long = System.currentTimeMillis): F[Either[MatcherError, List[ApiOrderBookHistoryItem]]] = tryParseJson {
        sttp
      ...
      override def tryOrderHistoryWithApiKey(owner: Address,
                                             activeOnly: Option[Boolean] = None): F[Either[MatcherError, List[ApiOrderBookHistoryItem]]] = tryParseJson {
        sttp
