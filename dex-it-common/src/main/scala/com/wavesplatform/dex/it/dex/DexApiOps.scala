...
object DexApiOps {
  ...
  implicit final class ExplicitGetDexApiOps[F[_]: CanExtract: Functor](val self: DexApi[F]) {
    ...
    def cancel(owner: KeyPair, assetPair: AssetPair, id: Order.Id): F[HttpSuccessfulSingleCancel] =
      explicitGet(self.tryCancel(owner, assetPair, id))
    ...
    def deleteOrderBook(assetPair: AssetPair): F[HttpMessage] = explicitGet(self.tryDeleteOrderBook(assetPair))
