...
import com.wavesplatform.dex.it.api.domain.{AssetBalanceResponse, ConnectedPeersResponse, WavesBalanceResponse}
...
object NodeApiOps {
  final implicit class ExplicitGetNodeApiOps[F[_]: Functor](val self: NodeApi[F])(implicit E: CanExtract[F]) {
    ...
    def transactionInfo(id: ByteStr): F[Option[Transaction]] = self.tryTransactionInfo(id).map {
      case Right(r) => Some(r)
      case Left(e) =>
        if (e.error == ApiError.TransactionDoesNotExist.id) None
