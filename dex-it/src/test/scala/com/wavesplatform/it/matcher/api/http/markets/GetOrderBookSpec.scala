...
class GetOrderBookSpec extends MatcherSuiteBase with TableDrivenPropertyChecks with RawHttpChecks {
  ...
  "GET /matcher/orderbook/{amountAsset}/{priceAsset}?depth={depth} " - {
    ...
    forAll(Table(
      ...
      ("2147483648", "More than Int.Max", "Provided depth in not correct, reason: Depth value must be an Integer"),
      ("100.0", "Double", "Provided depth in not correct, reason: Depth value must be an Integer"),
      ("incorrect", "Alphabetical", "Provided depth in not correct, reason: Depth value must be an Integer"),
      ("-1", "Less than zero", "Provided depth in not correct, reason: Depth value must be positive")
