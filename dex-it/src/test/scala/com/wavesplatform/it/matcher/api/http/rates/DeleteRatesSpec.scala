...
class DeleteRatesSpec extends MatcherSuiteBase with ApiKeyHeaderChecks {
  ...
  "DELETE /matcher/settings/rates/{assetId}" - {
    ...
    "should return error when rate was not specified" in {
      validateMatcherError(dex1.rawApi.deleteRate(usd), StatusCode.NotFound, 20971529, s"The rate for the asset $UsdId was not specified")
    }

    "should return an error for unexisted asset" in {
      validateMatcherError(dex1.rawApi.deleteRate("AAA", Map("X-API-Key" -> apiKey)), StatusCode.NotFound, 11534345, "The asset AAA not found")
    }

    "should return an error when user try to update Waves rate" in {
      validateMatcherError(dex1.rawApi.deleteRate(Waves), StatusCode.BadRequest, 20971531, "The rate for WAVES cannot be changed")
    }
