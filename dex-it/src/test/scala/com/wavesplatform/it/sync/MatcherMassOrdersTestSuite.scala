...
class MatcherMassOrdersTestSuite extends MatcherSuiteBase {
  ...
  "Create orders with statuses FILL, PARTIAL, CANCELLED, ACTIVE" - {
    "Place initial orders" in {
      ...
      dex1.api.cancelOneOrAllInPairOrdersWithSig(alice, aliceOrderToCancel) // TODO: remove this line in DEX-160
    ...
    "Mass orders creation with random lifetime. Active orders still in list" in {
      ...
      dex1.api.getOrderHistoryByPKWithSig(alice, activeOnly = Some(false)).length shouldBe 4
      dex1.api.getOrderHistoryByPKWithSig(alice, activeOnly = Some(true)).length shouldBe activeAliceOrders1

      dex1.api.getOrderHistoryByPKWithSig(bob, activeOnly = Some(false)).length shouldBe 2
      dex1.api.getOrderHistoryByPKWithSig(bob, activeOnly = Some(true)).length shouldBe 0

      val orderIds = dex1.api.getOrderHistoryByPKWithSig(alice).map(_.id)
      ...
      withClue("no flag") {
        val orderIdsAfterMatching1 = dex1.api.getOrderHistoryByPKWithSig(alice).map(_.id)
      ...
      withClue("activeOnly=false") {
        val orderIdsAfterMatching2 = dex1.api.getOrderHistoryByPKWithSig(alice, activeOnly = Some(false)).map(_.id)
      ...
      withClue("activeOnly=true") {
        val orderIdsAfterMatching3 = dex1.api.getOrderHistoryByPKWithSig(alice, activeOnly = Some(true)).map(_.id)
      ...
      dex1.api.getOrderHistoryByPKWithSig(bob).map(_.id).take(maxFinalizedOrders) should matchTo(bobsOrderIds.take(maxFinalizedOrders))
      dex1.api.getOrderHistoryByAssetPairAndPKWithSig(bob, wavesUsdPair).map(_.id).take(maxFinalizedOrders) should matchTo(
    ...
    "Filled and Cancelled orders should be after Partial And Accepted" in {
      val lastIdxOfActiveOrder =
        dex1.api.getOrderHistoryByPKWithSig(alice).lastIndexWhere(o =>
          o.status == Status.Accepted.name || o.status == Status.PartiallyFilled.name
        )
      val firstIdxOfClosedOrder =
        dex1.api.getOrderHistoryByPKWithSig(alice).indexWhere(o => o.status == Status.Filled.name || o.status == Status.Cancelled.name)
    ...
    "Accepted and PartiallyFilled orders should be sorted by timestamp." in {
      val activeAndPartialOrders =
        dex1.api
          .getOrderHistoryByPKWithSig(alice)
    ...
    "Filled and Cancelled orders should be sorted by timestamp." in {
      val filledAndCancelledOrders = dex1.api.getOrderHistoryByPKWithSig(alice)
    ...
    "check order history orders count after fill" in {
      val aliceAllActiveOrders = dex1.api.getOrderHistoryByPKWithSig(alice, activeOnly = Some(true))
      val aliceAllOrders = dex1.api.getOrderHistoryByPKWithSig(alice)
      val expectedAllOrders = maxFinalizedOrders + aliceAllActiveOrders.size
      aliceAllOrders.size shouldBe expectedAllOrders

      val alicePairActiveOrders = dex1.api.getOrderHistoryByAssetPairAndPKWithSig(alice, wavesUsdPair, activeOnly = Some(true))
      val alicePairOrders = dex1.api.getOrderHistoryByAssetPairAndPKWithSig(alice, wavesUsdPair)
