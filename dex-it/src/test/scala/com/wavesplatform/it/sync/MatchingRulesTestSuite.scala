...
class MatchingRulesTestSuite extends MatcherSuiteBase {
  ...
  "Start offset depends of order placing and cancelling but matching" in {
    ...
    dex1.api.orderBook(wctUsdPair).asks shouldBe Seq(ApiLevelAgg(amount, 15 * price))
    ...
    dex1.api.orderBook(wctUsdPair).bids shouldBe Seq(ApiLevelAgg(amount, 8 * price))
    ...
    dex1.api.orderBook(wctUsdPair).bids shouldBe Seq(ApiLevelAgg(amount, 7 * price))
  ...
  "Orders should be cancelled correctly when matcher rules are changed" in {
    ...
    dex1.api.orderBook(wctUsdPair).bids shouldBe Seq(ApiLevelAgg(2 * amount, 7 * price))
  ...
  "Matching on the same price level" in {
    ...
    dex1.api.orderBook(wctUsdPair).asks shouldBe Seq(ApiLevelAgg(amount, 5 * price))
  ...
  "Matching with old orders after tick size enabled" in {
    ...
    dex1.api.orderBook(wctUsdPair).asks shouldBe Seq(ApiLevelAgg(amount, 20 * price))
    ...
    withClue("partially filled order cancellation") {
      dex1.api.orderBook(wctUsdPair).bids shouldBe Seq(ApiLevelAgg(amount, 12 * price))
      dex1.api.reservedBalance(alice)(Waves) shouldBe matcherFee / 2
  ...
  "Matching orders of same price but neighbors levels" in {
    ...
    dex1.api.orderBook(wctUsdPair).asks shouldBe Seq(ApiLevelAgg(amount, 24 * price))
      ...
      .foreach {
        case (pair: AssetPair, amount: Long, price: Long) =>
          withClue(pair) {
            ...
            dex1.api.orderBook(pair).asks shouldBe Seq(ApiLevelAgg(amount, 24 * price))
            val bestBidOrderId = mkOrder(bob, pair, BUY, amount, 17 * price, matcherFee)
            dex1.api.place(bestBidOrderId)

            dex1.api.orderBook(pair).bids shouldBe Seq(ApiLevelAgg(amount, 12 * price))
