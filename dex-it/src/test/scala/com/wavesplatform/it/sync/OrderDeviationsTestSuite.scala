    ...
    "out of deviation bounds" - {
      "-- too low" - {
        for (assetPair <- Seq(wavesBtcPair, ethWavesPair, scriptAssetsPair)) s"$assetPair" in {
          ...
          dex1.api.getReservedBalanceWithApiKey(bob) shouldBe Map(assetPair.priceAsset -> 300600000L)

          dex1.api.cancelOneOrAllInPairOrdersWithSig(bob, bestBidOrder)
        ...
        s"$wavesUsdPair" in {
          ...
          dex1.api.getReservedBalanceWithApiKey(bob) shouldBe Map(wavesUsdPair.priceAsset -> 300600L)
  ...
  "sell orders price is" - {
    "in deviation bounds" - {
      ...
      for (assetPair <- Seq(wavesBtcPair, ethWavesPair, scriptAssetsPair)) s"$assetPair" in {
        ...
        dex1.api.getReservedBalanceWithApiKey(alice) should matchTo(Map(assetPair.amountAsset -> 300000000000L, feeAsset -> 7 * matcherFee))
        dex1.api.getReservedBalanceWithApiKey(bob) should matchTo(Map(assetPair.priceAsset -> 300300000L))
    ...
    "out of deviation bounds" - {
      ...
      "-- too high" - {
        for (assetPair <- Seq(wavesBtcPair, ethWavesPair, scriptAssetsPair)) s"$assetPair" in {
          ...
          dex1.api.getReservedBalanceWithApiKey(alice) should matchTo(Map(assetPair.amountAsset -> 1000.waves, feeAsset -> 2 * matcherFee))
        ...
        s"$wavesUsdPair" in {
          ...
          dex1.api.getReservedBalanceWithApiKey(alice) shouldBe Map(wavesUsdPair.amountAsset -> 1000.waves, feeAsset -> 2 * 300)
  ...
  "orders fee is" - {
    ...
    "out of deviation bounds" - {
      for (assetPair <- Seq(wavesBtcPair, ethWavesPair, scriptAssetsPair)) s"$assetPair" in {
        ...
        dex1.api.cancelOneOrAllInPairOrdersWithSig(alice, bestAskOrder)
      ...
      s"$wavesUsdPair" in {
        ...
        dex1.api.cancelOneOrAllInPairOrdersWithSig(bob, bestAskOrder)
