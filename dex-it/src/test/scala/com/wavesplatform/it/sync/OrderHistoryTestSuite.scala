...
class OrderHistoryTestSuite extends MatcherSuiteBase {
  ...
  "Order history should save fee info" in {
    val feeAsset = IssuedAsset(EthId)
    ...
    withClue("in placed and cancelled orderV3") {
      val aliceOrderV3Id = node.placeOrder(alice, wctUsdPair, BUY, amount, price, matcherFee, version = 3, matcherFeeAssetId = feeAsset).message.id
    ...
    withClue("in partially filled and cancelled orders") {
      val aliceOrderV3Id = node.placeOrder(alice, wctUsdPair, BUY, 2 * amount, price, matcherFee).message.id
      val bobOrderId =  node.placeOrder(bob, wctUsdPair, SELL, amount, price, matcherFee).message.id
      node.waitOrderInBlockchain(aliceOrderV3Id)
    ...
    withClue("in partially filled and cancelled orders of different versions") {
      val aliceOrderV3Id = node.placeOrder(alice, wctUsdPair, BUY, 2 * amount, price, matcherFee, version = 3, matcherFeeAssetId = feeAsset).message.id
      val bobOrderId =  node.placeOrder(bob, wctUsdPair, SELL, amount, price, matcherFee).message.id
      node.waitOrderInBlockchain(aliceOrderV3Id)
      node.orderStatus(aliceOrderV3Id, wctUsdPair).filledFee shouldBe Some(matcherFee / 2)
      node.orderStatus(bobOrderId, wctUsdPair).filledFee shouldBe Some(matcherFee)
    ...
    withClue("in partially filled orders with fractional filled amount") {
      val aliceOrderId = node.placeOrder(alice, wctUsdPair, BUY, 9 * amount, price, matcherFee, version = 3, matcherFeeAssetId = feeAsset).message.id
