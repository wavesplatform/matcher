...
class OrderRestrictionsTestSuite extends MatcherSuiteBase {
  ...
  "low amount order" in {
    assertBadRequestAndResponse(node.placeOrder(alice, wctUsdPair, BUY, 1, 100000000, matcherFee),
      s"The order's amount 0.01 $WctId does not meet matcher requirements: max amount = 100000000, min amount = 0.1, step amount = 0.1")
  }
  ...
  "wrong fraction amount" in {
    assertBadRequestAndResponse(node.placeOrder(alice, wctUsdPair, BUY, 15, 100000000, matcherFee),
      s"The order's amount 0.15 $WctId does not meet matcher requirements: max amount = 100000000, min amount = 0.1, step amount = 0.1")
  }

  "low price amount" in {
    assertBadRequestAndResponse(node.placeOrder(alice, wctUsdPair, BUY, 100000000, 25, matcherFee),
      "The order's price 0.00000025 does not meet matcher requirements: max price = 1000, min price = 0.0001, step price = 0.001")
  }

  "high price amount" in {
    assertBadRequestAndResponse(node.placeOrder(alice, wctUsdPair, BUY, 100000000, 1000000000000L, matcherFee),
      "The order's price 10000 does not meet matcher requirements: max price = 1000, min price = 0.0001, step price = 0.001")
  }
