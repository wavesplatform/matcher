...
class RatesTestSuite extends MatcherSuiteBase {
  ...
  "Rates can be handled via REST" in {
    ...
    // add rate for wct
    val addWctRate = dex1.api.upsertRate(wctAsset, wctRate)
    addWctRate._1 shouldBe StatusCodes.Created
    addWctRate._2.message shouldBe s"The rate $wctRate for the asset $wctStr added"
    dex1.api.rates shouldBe defaultRateMap + (wctAsset -> wctRate)

    // update rate for wct
    val updateWctRate = dex1.api.upsertRate(wctAsset, wctRateUpdated)
    ...
    dex1.api.rates shouldBe defaultRateMap + (wctAsset -> wctRateUpdated)
    ...
    // delete rate for wct
    dex1.api.deleteRate(wctAsset).message shouldBe s"The rate for the asset $wctStr deleted, old value = $wctRateUpdated"
    dex1.api.rates shouldBe defaultRateMap

    // delete unexisted rate
    dex1.api.tryDeleteRate(wctAsset) should failWith(20971529, MatcherError.Params(assetId = Some(wctStr)))
  }
  ...
  "Changing rates affects order validation" in {
    // set rate for btc
    dex1.api.upsertRate(btcAsset, 1)._1 shouldBe StatusCodes.Created
  ...
  "Rates are restored from the DB after matcher's restart" in {
    ...
    dex1.api.upsertRate(btcAsset, 1.1)._1 shouldBe StatusCodes.Created
