...
class RestOrderLimitTestSuite extends MatcherSuiteBase {
  ...
  private def activeOrders: List[Order.Id] = {
    val activeOrders = dex1Api.orderHistory(alice, activeOnly = Some(true)).map(_.id)
    dex1Api.orderHistoryWithApiKey(alice, activeOnly = Some(true)).map(_.id) should equal(activeOrders)
    activeOrders
  }

  private def allOrders: List[Order.Id] = {
    val allOrders = dex1Api.orderHistory(alice).map(_.id)
    dex1Api.orderHistoryWithApiKey(alice, activeOnly = Some(false)).map(_.id) should equal(allOrders)
    allOrders
  }

  private def activeOrdersBy(pair: AssetPair, n: KeyPair = alice): List[Order.Id] =
    dex1Api.orderHistoryByPair(n, pair, activeOnly = Some(true)).map(_.id)

  private def allOrdersBy(pair: AssetPair, n: KeyPair = alice): List[Order.Id] = dex1Api.orderHistoryByPair(n, pair).map(_.id)
  ...
  "Order History REST API methods should have limit for orders in response" in {
    ...
    List(active0, active1, partial1, filled1, cancelled1, active2, filled2, partial2, cancelled2).foreach(dex1Api.place)
    ...
    ).foreach(dex1Api.place)

    dex1Api.cancel(alice, cancelled1)
    dex1Api.cancel(alice, cancelled2)
    ...
    List(active3, active4, active5, active6).foreach(dex1Api.place)
    ...
    List(active7, active8, active9, active10).foreach(dex1Api.place)
    ...
    List(active11, active12, active13, active14, active15).foreach(dex1Api.place)
