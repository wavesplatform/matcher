...
class RoundingIssuesTestSuite extends MatcherSuiteBase {
  ...
  "reserved balance should not be negative" in {
    ...
    withClue("Alice's reserved balance before cancel")(dex1.api.getReservedBalanceByPK(alice) shouldBe empty)

    waitForOrderAtNode(counter)
    dex1.api.cancelOrder(bob, counter)

    withClue("Bob's reserved balance after cancel")(dex1.api.getReservedBalanceByPK(bob) shouldBe empty)
  }
