...
import com.wavesplatform.dex.domain.transaction.{ExchangeTransaction => ExchangeTransactionInternal}
...
class RoundingIssuesTestSuite extends MatcherSuiteBase {
  ...
  "should correctly fill an order with small amount" in {
    ...
    exchangeTx.price() shouldBe ExchangeTransactionInternal.convertPrice(counter.price, assetDecimalsMap(Waves), assetDecimalsMap(usd)).value
