...
class WsOrderBookStreamTestSuite extends WsSuiteBase {
  ...
  "Order book stream should" - {
    ...
    "correctly send changed tick-size" in {
      ...
      eventually {
        val buffer = wsc.receiveAtLeastN[WsOrderBook](1)
    ...
    "be opened even if there is no such order book" in {
      ...
      val snapshot = wsc.receiveAtLeastN[WsOrderBook](1).head
      ...
      wsc.receiveAtLeastN[WsOrderBook](1).head.asks should matchTo(TreeMap(231.0 -> 10.0))
    }
  }
  ...
  "Bugs" - {
    "DEX-814 Connections can affect each other" in {
      ...
      eventually {
        val buffer = mainWsc.receiveAtLeastN[WsOrderBook](1)
