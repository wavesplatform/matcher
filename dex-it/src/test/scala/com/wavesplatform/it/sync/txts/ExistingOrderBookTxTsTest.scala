...
final class ExistingOrderBookTxTsTest extends MatcherSuiteBase {
  ...
  "Existing orderBook" - {
    "should create tx with same ts if feature start offset isn't reached" in {
      ...
      dex1.api.waitForTransactionsByOrder(bigOrder, 2).map(_.timestamp()).toSet.size shouldBe 1
    }

    "should create tx with different ts if feature start offset isn't reached" in {
      val orders = Seq(mkOrder(alice, wavesUsdPair, OrderType.BUY, 10.waves, 5.usd), mkOrder(alice, wavesUsdPair, OrderType.BUY, 15.waves, 9.usd))
      orders.foreach(dex1.api.place)
      ...
      placeAndAwaitAtDex(bigOrder, Status.Filled)
      dex1.api.waitForTransactionsByOrder(bigOrder, 2).map(_.timestamp()).toSet.size shouldBe 2
    }
  }
