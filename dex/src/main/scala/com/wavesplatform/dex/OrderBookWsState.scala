...
case class OrderBookWsState(wsConnections: Map[ActorRef[WsOrderBook], (UUID, Long)], changes: WsOrderBook) {

  def addSubscription(x: ActorRef[WsOrderBook], id: UUID): OrderBookWsState = copy(wsConnections = wsConnections + (x -> (id -> 0L)))
  ...
  def withLastTrade(update: WsOrderBook.Update, x: LastTrade): OrderBookWsState =
    if (hasSubscriptions) copy(changes = update.withLastTrade(changes, x)) else this
  def withLevelChanges(update: WsOrderBook.Update, xs: LevelAmounts): OrderBookWsState =
