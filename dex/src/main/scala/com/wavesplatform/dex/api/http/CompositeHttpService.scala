...
class CompositeHttpService(apiTypes: Set[Class[_]], routes: Seq[ApiRoute], settings: RestAPISettings) extends ScorexLogging {
  ...
  private val swaggerRoute: Route = measureResponse("swaggerEndpoints") {
  ...
  val compositeRoute: Route = passRequestIdAttributeToResponse(extendRoute(routes.map(_.route).reduce(_ ~ _)) ~ swaggerRoute ~ complete(StatusCodes.NotFound))
