...
trait LevelDb[F[_]] {
  ...
  def scanOver[R](prefix: Short)(acc: R)(f: (R, DBEntry) => R): F[R] = scanOver(Shorts.toByteArray(prefix))(acc)(f)
  def scanOver[R](prefix: Array[Byte])(acc: R)(f: (R, DBEntry) => R): F[R]
...
object LevelDb {
  ...
  def async(db: DB, mtex: ExecutionContext, stex: ExecutionContext): LevelDb[Future] = new LevelDb[Future] {
     ...
     */
    override def readOnly[A](f: ReadOnlyDb => A): Future[A] = Future(db.readOnly(f))(mtex)

    /**
     * Do not chain with map/flatMap/etc. See above
     */
    override def readWrite[A](f: ReadWriteDb => A): Future[A] = Future(db.readWrite(f))(stex)
    override def get[A](key: Key[A]): Future[A] = Future(db.get(key))(mtex)
    override def put[A](key: Key[A], value: A): Future[Unit] = Future(db.put(key.keyBytes, key.encode(value)))(mtex)
    override def delete[A](key: Key[A]): Future[Unit] = Future(db.delete(key.keyBytes))(mtex)
    override def has(key: Key[_]): Future[Boolean] = Future(db.has(key))(mtex)
    override def scanOver[R](prefix: Array[Byte])(acc: R)(f: (R, DBEntry) => R): Future[R] = Future(db.scanOver(prefix)(acc)(f))(mtex)
    override def iterateOver(prefix: Array[Byte])(f: DBEntry => Unit): Future[Unit] = Future(db.iterateOver(prefix)(f))(mtex)
  }
  ...
  def sync(db: DB): LevelDb[Id] = new LevelDb[Id] {
    ...
    override def scanOver[R](prefix: Array[Byte])(acc: R)(f: (R, DBEntry) => R): R = db.scanOver(prefix)(acc)(f)
