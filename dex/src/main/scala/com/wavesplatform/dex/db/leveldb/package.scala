...
package object leveldb extends ScorexLogging {
  ...
  implicit class ByteArrayDataOutputExt(val output: ByteArrayDataOutput) extends AnyVal {
    ...
    def writeByteStr(s: ByteStr): Unit = output.write(s.arr)
  ...
  implicit class ByteArrayDataInputExt(val input: ByteArrayDataInput) extends AnyVal {
    ...
    def readByteStr(len: Int): ByteStr = ByteStr(readBytes(len))

    def readSignature: ByteStr   = readByteStr(crypto.SignatureLength)
    def readPublicKey: PublicKey = PublicKey(readBytes(crypto.KeyLength))
  }
