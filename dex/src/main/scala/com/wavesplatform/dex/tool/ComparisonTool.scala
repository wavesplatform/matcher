...
class ComparisonTool(settings: Settings) extends ScorexLogging {
  ...
  implicit private val ec = ExecutionContext.fromExecutor(Executors.newFixedThreadPool(5))
  ...
  private def runTradableBalanceChecks(): Unit = {
    val xs =
      for {
        ...
        .get(uri"$baseUri/tradableBalance/${assetPair.amountAsset}/${assetPair.priceAsset}/${pk.toAddress.stringRepr}")
        .response(asString("UTF-8"))
        .send()
        .map(r => (pk, baseUri, parse(r)))
    ...
    Future
      ...
      .onComplete {
        ...
        case Success(responses) =>
          responses.groupMap(_._1) { case (pk, server, response) => (server, response) }
            .foreach { case (pk, responses) =>
              val (str, areSame) = compareResponses(NonEmptyList.fromList(responses).getOrElse(throw new RuntimeException("Improsibru!")))
              val message = s"${pk.toAddress}: $str"
  ...
  private def compareResponses(xs: NonEmptyList[(Uri, TradableBalanceCheckResponse)]): (String, Boolean) = {
    ...
    xs.foldLeft((s"$reference", true)) { case (r @ (str, same), (server, response)) =>
...
object ComparisonTool {
  ...
  case class ChecksSettings(interval: FiniteDuration, strike: Int)
