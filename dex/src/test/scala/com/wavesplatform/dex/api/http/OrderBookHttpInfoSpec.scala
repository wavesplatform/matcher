  ...
  "OrderBookHttpInfo" - {
    "getHttpView" - {
      "should return the nearest depth cache" - {
        ...
        def get(depth: Option[Int]): OrderBookResult =
          orderBookFrom(Await.result(orderBookHttpInfo.getHttpView(pair, MatcherModel.Normalized, depth), 5.seconds))
    ...
    "Settings.nearestBigger" - {
      def mkSettings(depthRanges: List[Int], defaultDepth: Option[Int]) = OrderBookHttpInfo.Settings(depthRanges, defaultDepth)
