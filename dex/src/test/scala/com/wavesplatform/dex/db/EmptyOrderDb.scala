...
class EmptyOrderDb[F[_]: Applicative] private () extends OrderDb[F] {
  ...
  override def saveOrderInfo(id: Order.Id, oi: OrderInfo[OrderStatus.Final]): F[Unit] = ().pure[F]
  override def saveOrderInfoForHistory(id: Id, sender: Address, oi: OrderInfo[OrderStatus.Final]): F[Unit] = ().pure[F]
