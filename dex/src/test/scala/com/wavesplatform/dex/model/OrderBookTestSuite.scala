...
class OrderBookTestSuite
  ...
  "place buy orders with different prices" in {
    ...
    val (ob, events) = OrderBook.empty.appendAll(List(ord1, ord2, ord3), now)
    ob.allOrders.toSeq.map(_._2) should matchTo(Seq(ord2, ord1, ord3))
  }
    ...
    val gen = Gen
      ...
      .map {
        case (tickSize, askPrices, bidPrices) =>
          val asks = askPrices.map(x => LimitOrder(sell(pair, 984651354686L, x)))
          val bids = bidPrices.map(x => LimitOrder(buy(pair, 1583290045643L, x)))
          (tickSize, OrderBook.empty.appendAll(asks ++ bids, now, tickSize = tickSize)._1)
      }
  ...
  "place buy and sell orders with prices different from each other less than tick size in one level" in {
    ...
    val (ob, _) = OrderBook.empty.appendAll(
    ...
    OrderBook.empty.append(LimitOrder(sellTickSizeOrder), ntpNow, tickSize = normalizedTickSize)._1.asks should matchTo(
      TreeMap[Price, Level](
        Seq(
  ...
  "place matchable orders with and without tick size" in {
    ...
    withClue("matchable orders should not be matched with tick size:\n") {
      val (ob, events) = ob1.appendAll(List(counterSellOrder, submittedBuyOrder), now, tickSize = normalizedTickSize)
