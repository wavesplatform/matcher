 ...
 */
case class WavesChain(history: List[WavesBlock], height: Int) {
  ...
  private def withFullBlock(block: WavesBlock): Either[String, WavesChain] = history match {
    case Nil => WavesChain(block :: history, block.ref.height).asRight
    case prev :: _ =>
      if (block.ref.height == prev.ref.height + 1 && block.reference == prev.ref.id) {
        ...
        WavesChain(newHistory, block.ref.height).asRight
  ...
  private def withMicroBlock(microBlock: WavesBlock): Either[String, WavesChain] = history match {
    ...
    case prev :: _ =>
      if (microBlock.ref.height == prev.ref.height && microBlock.reference == prev.ref.id)
        WavesChain(microBlock :: history, microBlock.ref.height).asRight
      else
  ...
  def withoutLast: WavesChain = {
    ...
    WavesChain(updatedHistory, math.max(0, height - 1))
  }
  ...
  def dropAfter(ref: BlockRef): (WavesChain, List[WavesBlock]) = {
    ...
    (WavesChain(commonHistory, ref.height), droppedBlocks)
  }
  ...
  def dropAfter(height: Int): (WavesChain, List[WavesBlock]) = {
    ...
    (WavesChain(commonHistory, height), droppedBlocks)
  }

  def dropAll: (WavesChain, List[WavesBlock]) = (WavesChain(Nil, history.lastOption.fold(height)(x => math.max(0, x.ref.height - 1))), history)
