  ...
  "StatusTransitions" - {
    "Normal +" - {
      "Appended ->" - {
        ...
        "TransientRollback" - {
          "because of an invalid new block" in {
            ...
            val init = Normal(WavesChain(List(block1), block1.ref.height))
            val event = Appended(block = block1, forgedTxIds = Nil)
            StatusTransitions(init, event) should matchTo(StatusUpdate(
              newStatus = TransientRollback(
                fork = WavesFork(init.main, WavesChain(List.empty, 1), connected = false),
      ...
      "[RolledBack] -> Normal, where [RolledBack] is" - {
        ...
        val init = Normal(WavesChain(List(block2, block1), 2))

        def test(to: RolledBack.To): Unit = StatusTransitions(init, RolledBack(to)) should matchTo(StatusUpdate(
          newStatus = TransientRollback(
            fork = WavesFork(init.main, WavesChain(List(block1), block1.ref.height), connected = true),
            utxEventsStash = Queue.empty
          ),
          updatedLastBlockHeight = StatusUpdate.LastBlockHeight.NotChanged,
          requestNextBlockchainEvent = true
    ...
    "TransientRollback +" - {
      ...
      val init = TransientRollback(
        fork = WavesFork(
          origChain = WavesChain(List(block2A, block1), block2A.ref.height),
          forkChain = WavesChain(List(block2B, block1), block2B.ref.height),
          connected = true
        ),
        utxEventsStash = Queue.empty
      )
      ...
      "Appended ->" - {
        "Normal" in {
          ...
          StatusTransitions(init, event) should matchTo(StatusUpdate(
            newStatus = Normal(
              main = WavesChain(List(microBlock, block2B, block1), 2)
            ),
        ...
        "TransientRollback" - {
          "because of an unrelated new block" in {
            ...
            StatusTransitions(init, event) should matchTo(StatusUpdate(
              newStatus = TransientRollback(
                ...
                fork = WavesFork(init.fork.origChain, WavesChain(List(block1), block1.ref.height), connected = true),
                utxEventsStash = Queue.empty
              ),
              updatedLastBlockHeight = StatusUpdate.LastBlockHeight.RestartRequired(2)
            ))
          ...
          "because the resolving process haven't yet completed" in {
            ...
            val init = TransientRollback(
              fork = WavesFork(
                origChain = WavesChain(List(block3, block2A, block1), block3.ref.height),
                forkChain = WavesChain(List(block1), block1.ref.height),
                connected = true
              ),
              utxEventsStash = Queue.empty
            )
            ...
            StatusTransitions(init, event) should matchTo(StatusUpdate(
              newStatus = TransientRollback(
                fork = WavesFork(init.fork.origChain, WavesChain(List(block2B, block1), block2B.ref.height), connected = true),
                utxEventsStash = Queue.empty
              ),
              requestNextBlockchainEvent = true
            ))
    ...
    "TransientResolving +" - {
      ...
      val init = TransientResolving(
        main = WavesChain(List(microBlock, block), microBlock.ref.height),
