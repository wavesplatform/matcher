  ...
  "StatusTransitions" - {
    "Normal +" - {
      "Appended ->" - {
        "Normal" in {
          ...
          StatusTransitions(init, event) should matchTo(StatusUpdate(
            newStatus = Normal(WavesChain(Vector(newBlock), 1, 99)),
        ...
        "TransientRollback" - {
          "because of an invalid new block" in {
            ...
            val init = Normal(WavesChain(Vector(block1), block1.ref.height, 99))
      ...
      "[RolledBack] -> Normal, where [RolledBack] is" - {
        ...
        val init = Normal(WavesChain(Vector(block2, block1), 2, 98))

        def test(to: RolledBack.To): Unit = StatusTransitions(init, RolledBack(to)) should matchTo(StatusUpdate(
          newStatus = TransientRollback(
            fork = WavesFork(init.main, WavesChain(Vector(block1), block1.ref.height, 99)),
            utxEventsStash = Queue.empty
          ),
          updatedLastBlockHeight = StatusUpdate.LastBlockHeight.NotChanged,
          requestNextBlockchainEvent = true
    ...
    "TransientRollback +" - {
      ...
      val init = TransientRollback(
        fork = WavesFork(
          origChain = WavesChain(Vector(block2A, block1), block2A.ref.height, 98),
          forkChain = WavesChain(Vector(block2B, block1), block2B.ref.height, 98)
        ),
        utxEventsStash = Queue.empty
      )
      ...
      "Appended ->" - {
        "Normal" in {
          ...
          StatusTransitions(init, event) should matchTo(StatusUpdate(
            newStatus = Normal(
              main = WavesChain(Vector(microBlock, block2B, block1), 2, 98)
            ),
        ...
        "TransientRollback" - {
          "because of an unrelated new block" in {
            ...
            StatusTransitions(init, event) should matchTo(StatusUpdate(
              newStatus = TransientRollback(
                ...
                fork = WavesFork(init.fork.origChain, WavesChain(Vector(block1), block1.ref.height, 99)),
                utxEventsStash = Queue.empty
              ),
              updatedLastBlockHeight = StatusUpdate.LastBlockHeight.RestartRequired(2)
            ))
          ...
          "because the resolving process haven't yet completed" in {
            ...
            val init = TransientRollback(
              fork = WavesFork(
                origChain = WavesChain(Vector(block3, block2A, block1), block3.ref.height, 97),
                forkChain = WavesChain(Vector(block1), block1.ref.height, 99)
              ),
              utxEventsStash = Queue.empty
            )
            ...
            StatusTransitions(init, event) should matchTo(StatusUpdate(
              newStatus = TransientRollback(
                fork = WavesFork(init.fork.origChain, WavesChain(Vector(block2B, block1), block2B.ref.height, 98)),
                utxEventsStash = Queue.empty
              ),
              requestNextBlockchainEvent = true
            ))
    ...
    "TransientResolving +" - {
      ...
      val init = TransientResolving(
        main = WavesChain(Vector(microBlock, block), microBlock.ref.height, 99),
